...
def test_redactData(spark_session):
    ...
    def mock_api_call(*args, **kwargs):
        ...
        expected = match[2] if match else raw_input
    ...
    with patch("src.engine.llm_client.OpenAI") as MockOpenAI:
        ...
        # Compare using key lookup rather than assuming index order
        expected_map = {item[1]: item[2] for item in mock_test_data}

        for row in results:
            input_val = row["input_text"]
            actual = row["scrubbed_text"]
            expected = expected_map[input_val]
            assert actual == expected, f"Failed for input: {input_val}\nExpected: {expected}\nGot: {actual}"